...
export class TestTreeComponent {
  ...
  getTreeData() {
    this.dataService
      .getAll(
        `${this.baseUrl + ApiEndpointType.CustomCoursesTestForTreeForCreator}/${
          this.skip
        }/${this.take}`
      )
      .then((x: any) => {
  ...
  // openModal() {
  //   const dialogRef = this.dialog.open(AdminTestsAddComponent,{data:'custom'});
  ...
  editNode(node: any) {
    switch(node.level){
      ...
      case 1:{
        ...
        this.addAnswer = false;
        this.editedTestQuestionNumber = false;
        this.testQuestionAnsId = 0;
        break;
      }
      case 2:{
